...
class Schema {
  ...
  async load () {
    ...
    try {
      this.schemaObject = await (await webfetch(url)).json()
    } catch (e) {
    ...
    if (this.schemaObject.type === 'json') {
      ...
      this.validate = createBinaryValidator()
    }
  }
...
function createBinaryValidator () {
  const validateBinary = function (value) {
    if (!Buffer.isBuffer(value)) {
      validateBinary.errors = [{message: 'Must be a binary buffer'}]
      return false
    }
    return true
  }
  validateBinary.errors = undefined
