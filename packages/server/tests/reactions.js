...
test('self indexes', async t => {
  ...
  await bob.login()
  let res = await api.view.get('ctzn.network/views/reactions-to', {dbUrl: bob.posts[0].dbUrl})
  t.is(res.subject.dbUrl, bob.posts[0].dbUrl)
  t.is(res.reactions.like.length, 2)
  t.is(res.reactions.woah.length, 1)

  await bob.login()
  res = await api.view.get('ctzn.network/views/reactions-to', {dbUrl: bob.posts[1].dbUrl})
  t.is(res.subject.dbUrl, bob.posts[1].dbUrl)
  t.is(res.reactions.like.length, 1)
  t.is(res.reactions.woah.length, 2)

  await carla.login()
  res = await api.view.get('ctzn.network/views/reactions-to', {dbUrl: bob.posts[0].dbUrl})
  t.is(res.subject.dbUrl, bob.posts[0].dbUrl)
  t.is(res.reactions.like.length, 2)
  t.is(res.reactions.woah.length, 1)
